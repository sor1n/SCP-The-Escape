...
public class Raycast
{
	...
	static final int PROJECTIONPLANEWIDTH = Main.WIDTH;
	static final int PROJECTIONPLANEHEIGHT = Main.HEIGHT;
	...
	public void tick(int delta)
	{
		...
		if(Keyboard.isKeyDown(Keyboard.KEY_UP)) fKeyUp = true;
		else fKeyUp = false;
		if(Keyboard.isKeyDown(Keyboard.KEY_DOWN)) fKeyDown = true;
		else fKeyDown = false;
		if(Keyboard.isKeyDown(Keyboard.KEY_RIGHT)) fKeyRight = true;
		else fKeyRight = false;
		if(Keyboard.isKeyDown(Keyboard.KEY_LEFT)) fKeyLeft = true;
		else fKeyLeft = false;
		...
		{
			if((fPlayerArc += ANGLE5 / 2) >= ANGLE360) fPlayerArc -= ANGLE360;
		}
...
//			fOffscreenGraphics.fillRect(0, r, PROJECTIONPLANEWIDTH, 15);
			GL11.glDisable(GL11.GL_TEXTURE_2D);
			Render.drawRectangle(0, r, PROJECTIONPLANEWIDTH, 15, c, 20, 20);
			c += 0.3f;
		}
	}
	...
	public void drawOverheadMap() //TODO
	{
		...
		for(int u = 0; u < MAP_WIDTH; u++)
		{
			...
			{
//				if(fMap[v*MAP_WIDTH+u]==W) fOffscreenGraphics.setColor(Color.cyan);
//				else fOffscreenGraphics.setColor(Color.black);
				//fOffscreenGraphics.fillRect(PROJECTIONPLANEWIDTH+(u*fMinimapWidth), (v*fMinimapWidth), fMinimapWidth, fMinimapWidth);
				if(fMap[v * MAP_WIDTH + u] == W) Render.drawRectangle(PROJECTIONPLANEWIDTH-(u*fMinimapWidth + fMinimapWidth) - fMinimapWidth, (v*fMinimapWidth) + fMinimapWidth, fMinimapWidth, fMinimapWidth, 255, 255, 255);
				else Render.drawRectangle(PROJECTIONPLANEWIDTH-(u*fMinimapWidth + fMinimapWidth) - fMinimapWidth, (v*fMinimapWidth) + fMinimapWidth, fMinimapWidth, fMinimapWidth, 10, 10, 10);
			}
		}
		fPlayerMapX = PROJECTIONPLANEWIDTH-(int)(((float)fPlayerX/(float)TILE_SIZE) * fMinimapWidth) - fMinimapWidth;
		fPlayerMapY = (int)(((float)fPlayerY/(float)TILE_SIZE) * fMinimapWidth) + fMinimapWidth;
	}
	...
	//*******************************************************************//
	public void drawRayOnOverheadMap(float x, float y) //TODO
	{
	...
	public void render()
	{
		...
		for (castColumn=0; castColumn<PROJECTIONPLANEWIDTH; castColumn+=5)
		{
			...
			Render.drawRectangle(castColumn, topOfWall, 5, projectedWallHeight, brightness, brightness, brightness);
